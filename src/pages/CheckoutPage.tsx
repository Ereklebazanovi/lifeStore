...
const CheckoutPage: React.FC = () => {
  ...
  const handlePreSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.phone || formData.phone.length !== 9 || !formData.phone.startsWith("5")) {
      showToast("გთხოვთ ჩაწეროთ სწორი ქართული მობილურის ნომერი (9 ციფრი)", "error");
      return;
    }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <div className="bg-white p-6 lg:p-8 rounded-2xl shadow-sm border border-stone-200">
              ...
              <form id="checkout-form" onSubmit={handlePreSubmit} className="space-y-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-stone-700 mb-1">გვარი *</label>
                    <input
                      type="text"
                      name="lastName"
                      required
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {/* Primary Phone */}
                  <div>
                    <label className="block text-sm font-medium text-stone-700 mb-1">მობილური *</label>
                    <PhoneInput
                  ...
                  <div>
                    <label className="block text-sm font-medium text-stone-700 mb-1">
                      დამატებითი ნომერი <span className="text-stone-400 text-xs font-normal">(არასავალდებულო)</span>
                    </label>
                    <PhoneInput
                ...
                {/* Email */}
                <div>
                  <label className="block text-sm font-medium text-stone-700 mb-1">ელ-ფოსტა *</label>
            ...
            <div className="bg-white p-6 lg:p-8 rounded-2xl shadow-sm border border-stone-200">
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-stone-700 mb-1">ქალაქი / რაიონი *</label>
                  <select
                    name="city"
                    value={formData.city}
                    onChange={handleChange}
                ...
                <div>
                  <label className="block text-sm font-medium text-stone-700 mb-1">ზუსტი მისამართი *</label>
                  <input
                    type="text"
                    name="address"
                    required
                ...
                <div>
                  <label className="block text-sm font-medium text-stone-700 mb-1">კომენტარი (არასავალდებულო)</label>
                  <textarea
                    name="comment"
                    rows={3}
              ...
              <div className="space-y-3">
                <label className="flex items-center p-4 border border-emerald-200 bg-emerald-50 rounded-xl cursor-pointer transition-all">
                  ...
                  <div className="ml-3 flex items-center gap-2">
                    ...
                    <span className="font-medium text-stone-900">ადგილზე გადახდა (ქეში/ბარათი)</span>
                  </div>
                </label>
                <label className="flex items-center p-4 border border-stone-200 rounded-xl opacity-60 cursor-not-allowed">
                  <input type="radio" disabled className="w-5 h-5 text-stone-400" />
                  <div className="ml-3">
                    <span className="font-medium text-stone-500">ონლაინ გადახდა (მალე დაემატება)</span>
                  </div>
                </label>
              </div>
            </div>
          ...
          <div className="lg:col-span-5">
            <div className="bg-white p-6 lg:p-8 rounded-2xl shadow-lg border border-stone-100 sticky top-24">
              <h2 className="text-xl font-bold text-stone-800 mb-6">შეკვეთის დეტალები</h2>
                ...
                {items.map((item) => (
                  <div key={item.productId} className="flex gap-4 items-start">
                    ...
                    <div className="flex-1">
                      <h4 className="text-sm font-bold text-stone-900 line-clamp-2">{item.product.name}</h4>
                      <div className="flex justify-between mt-1">
                        <span className="text-xs text-stone-500">{item.quantity} x ₾{item.product.price}</span>
                        <span className="text-sm font-semibold text-stone-700">₾{(item.quantity * item.product.price).toFixed(2)}</span>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div className="border-t border-stone-100 pt-4 space-y-3 mb-6">
                ...
                <div className="flex justify-between items-center text-stone-600">
                  ...
                  <span className={shippingCost === 0 ? "text-green-600 font-bold" : "text-stone-900 font-medium"}>
              ...
              {/* ✅ Terms Checkbox */}
              <div id="terms-container" className="flex items-start gap-3 mb-6 p-4 bg-stone-50 rounded-xl border border-stone-100 transition-all hover:border-emerald-100">
                <input
                  type="checkbox"
                  id="terms-checkbox"
                  checked={isAgreed}
                ...
                />
                <label htmlFor="terms-checkbox" className="text-sm text-stone-600 cursor-pointer select-none leading-relaxed">
                  ვეთანხმები <Link to="/terms" target="_blank" className="text-stone-900 font-bold hover:text-emerald-600 underline">მომსახურების პირობებს*</Link>
                </label>
              </div>
              ...
              <button
                ...
                className={`w-full font-bold py-4 rounded-xl flex items-center justify-center gap-2 transition-all duration-300 shadow-lg active:scale-95
                  ${isSubmitting || !isAgreed ? "bg-stone-300 text-stone-500 cursor-not-allowed" : "bg-stone-900 hover:bg-emerald-600 text-white"}`}
              >
                {isSubmitting ? <span>მუშავდება...</span> : (
                  <>
              ...
              {!isAgreed && (
                <p className="text-xs text-red-400 text-center mt-3 font-medium animate-pulse">* შეკვეთის გასაფორმებლად სავალდებულოა წესებზე თანხმობა</p>
              )}
            </div>
          </div>
        </div>

            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-8">
              <div>
                ...
                <h2 className="text-2xl lg:text-3xl font-bold text-stone-900">
                  {selectedFilter === "popular" ? "პოპულარული ნივთები" :
                   selectedFilter === "discounts" ? "ფასდაკლებები" :
                   selectedFilter === "new" ? "ახალი დამატებული" :
                   "ყველა პროდუქტი"}
                </h2>
                {filteredProducts.length !== products.filter(p => p.isActive !== false).length && (
                  <p className="text-stone-500 text-sm mt-1">
                    ნაჩვენებია {filteredProducts.length} პროდუქტი {products.filter(p => p.isActive !== false).length}-დან
                  </p>
                )}
              </div>
              <Link
          ...
          {isLoading ? (
            ...
            <div className="text-center py-24 bg-white rounded-3xl border border-stone-100">
              ...
              <h3 className="text-xl font-bold text-stone-900 mb-2">
                {searchTerm ? "ძებნის შედეგები ვერ მოიძებნა" : "კოლექცია მალე განახლდება"}
              </h3>
              <p className="text-stone-500">
                {searchTerm ? `"${searchTerm}" მოძებნა შედეგების გარეშე` : "პროდუქტები ემატება..."}
              </p>

                ...
                <div className="mt-3 p-4 bg-amber-50 border border-amber-200 rounded-xl">
                  {notifySuccess ? (
                    <div className="flex items-center gap-2 text-emerald-700 text-sm font-medium">
                      ...
                      მეილი დარეგისტრირდა! შეგატყობინებთ მარაგის შევსებისას.
                    </div>
                  ) : (
                    <>
                      <p className="text-sm text-stone-700 mb-3 font-medium">
                        🔔 შეგატყობინოთ, როცა დაემატება?
                      </p>
                      <form onSubmit={handleNotifySubmit} className="flex flex-col sm:flex-row gap-2">
                        <input
                          ...
                          className="flex-1 min-w-0 px-3 py-2.5 text-sm border border-stone-200 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none bg-white"
                        />
                        <button
                          type="submit"
                          disabled={notifySubmitting}
                          className="w-full sm:w-auto px-5 py-2.5 bg-stone-900 hover:bg-emerald-600 text-white text-sm font-bold rounded-lg transition-colors disabled:opacity-50 whitespace-nowrap"
                        >
                          {notifySubmitting ? "..." : "გაგზავნა"}
                        </button>
                      </form>
                    </>
                  )}
        ...
        <div id="reviews" className="px-4 lg:px-0 py-10 border-t border-stone-100 mt-6">
          ...
          {user && hasReviewed ? (
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 py-3.5 border-2 border-dashed border-stone-200 hover:border-emerald-400 hover:bg-emerald-50 rounded-xl text-stone-500 hover:text-emerald-600 font-medium text-sm transition-all group"
            >
...
🛒 შეუკვეთეთ ახლავე: https://lifestore.ge/product/${productSlug}`;
      ...
      {showReviewModal && product && (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-end sm:items-center justify-center overflow-hidden">
          <div className="bg-white w-full max-w-full sm:max-w-md sm:rounded-2xl rounded-t-2xl shadow-2xl overflow-hidden">
